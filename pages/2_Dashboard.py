...
.stNumberInput>div>div>input,.stTextInput>div>div>input,.stTextArea>div>div>textarea{
  ...
  color:#fff!important;border-radius:14px!important;backdrop-filter:blur(28px)!important;font-size:1.05rem!important;font-weight:500!important;}
...
.act-day{font-size:0.85rem;color:rgba(255,255,255,0.90)!important;font-weight:600;}
.cal-hdr-row{display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px;}
.cal-dow{font-size:0.85rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:rgba(255,255,255,0.95)!important;text-align:center;padding:4px 0;}
        ...
        for pair in _up_pairs:
            ...
            for _col_widget, cd in zip([up_c1, up_c2], pair):
                with _col_widget:
                    ...
                    _date_style = "color:#E50914;background:rgba(229,9,20,0.15);border:1px solid rgba(229,9,20,0.35);border-radius:5px;padding:1px 7px;" if _da==0 else "color:rgba(229,9,20,0.75);"
                    st.markdown(
                        ...
                        f"<div><div style='font-size:0.85rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:{_accent};margin-bottom:2px'>{'REST' if _rest else _mg2.split()[0].upper()[:8]}</div>"
